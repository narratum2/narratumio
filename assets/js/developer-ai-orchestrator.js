...
(function () {
    ...
    const DesignAgent = {
        ...
        run(ctx) {
            const findings = [];
            const d = ctx.document;
            // 1. Check color variables
            const rootStyle = getComputedStyle(d.documentElement);
            const expectedBg = '#0a1520';
            if (rootStyle.getPropertyValue('--bg-primary').trim().toLowerCase() !== expectedBg) {
                findings.push({ severity: 'warn', message: `Primary background colour should be ${expectedBg}.` });
            }
            // 2. Ensure hero section exists and contains the brand name
            if (!d.querySelector('h1') || !/Narratum/i.test(d.querySelector('h1').textContent)) {
                findings.push({ severity: 'error', message: 'Hero section with “Narratum” heading missing.' });
            }
            // 3. Verify star-field/constellation elements exist on desktop
            if (window.innerWidth > 768 && !d.querySelector('.gentle-stars') && !d.querySelector('.constellation')) {
                findings.push({ severity: 'warn', message: 'Star‑field background missing on desktop.' });
            }
            // 4. Check for font variables
            const expectedSerif = "'Cormorant Garamond', serif";
            const expectedSans = "'Inter', sans-serif";
            if (rootStyle.getPropertyValue('--font-serif').trim() !== expectedSerif) {
                findings.push({ severity: 'warn', message: `Serif font should be ${expectedSerif}.` });
            }
            if (rootStyle.getPropertyValue('--font-sans').trim() !== expectedSans) {
                findings.push({ severity: 'warn', message: `Sans-serif font should be ${expectedSans}.` });
            }
            // 5. Check for key layout variables
            const expectedPadding = "120px";
            if (rootStyle.getPropertyValue('--section-padding').trim() !== expectedPadding) {
                findings.push({ severity: 'warn', message: `Section padding should be ${expectedPadding}.` });
            }
            const expectedWidth = "1200px";
            if (rootStyle.getPropertyValue('--content-width').trim() !== expectedWidth) {
                findings.push({ severity: 'warn', message: `Content width should be ${expectedWidth}.` });
            }
            // 6. Other design tests (add as needed)
            return { status: findings.length ? 'warn' : 'ok', findings };
        }
    };
    ...
    const BackgroundAgent = {
        ...
        baseIntervalMinutes: 120, // Run less frequently as it's comprehensive
        timeoutMs: 10000,
        async run(ctx) {
            if (window.narratumAgent && typeof window.narratumAgent.triggerAnalysis === 'function') {
                try {
                    const report = await window.narratumAgent.triggerAnalysis();
                    const findings = [];
                    
                    if (report.criticalIssues > 0) {
                        findings.push({ severity: 'error', message: `${report.criticalIssues} critical issue(s) detected.` });
                    }
                    if (report.optimizationOpportunities > 0) {
                        findings.push({ severity: 'warn', message: `${report.optimizationOpportunities} optimization opportunities found.` });
                    }

                    return { status: findings.length ? 'warn' : 'ok', findings };
                } catch (error) {
                    return { status: 'error', findings: [{ severity: 'error', message: 'Background analysis failed.' }] };
                }
            }
    ...
    function bootstrap() {
        try {
            ...
            orchestrator.registerAgent(BackgroundAgent);
            orchestrator.init();
